...
function getConnection(name: string): Pool {
...
async function removeConnection(name: string): Promise<void> {
  const conn = connections.get(name);
  if (!conn) {
...
async function handleRemoveConnection(name: string): Promise<string> {
...
async function handleTestConnection(name: string): Promise<string> {
  const pool = getConnection(name);
...
async function handleListSchemas(connection: string): Promise<string> {
...
async function handleListTables(connection: string, schema: string = "public"): Promise<string> {
  const db = getConnection(connection);
  const result = await db.query(
    `
...
async function handleQuery(
  ...
  if (!isReadOnlyQuery(sql)) {
    throw new Error(
      "Only read-only queries (SELECT, WITH, EXPLAIN) are allowed. Modification statements are not permitted."
    );
  }
...
async function handleGetTableCount(
  connection: string,
  table: string,
  schema: string = "public"
): Promise<string> {
...
async function handleGetForeignKeys(
  connection: string,
  table: string,
  schema: string = "public"
): Promise<string> {
...
async function handleCompareSchemas(
  ...
  const sourceColumns = new Map(sourceResult.rows.map((r: any) => [r.column_name, r]));
  const targetColumns = new Map(targetResult.rows.map((r: any) => [r.column_name, r]));
...
async function handleCompareRowCounts(
  sourceConnection: string,
  targetConnection: string,
  tables: string[] = [],
  schema: string = "public"
  ...
  if (tableList.length === 0) {
    ...
    tableList = tablesResult.rows.map((r: any) => r.table_name);
  }
...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        case "add_connection":
          result = await handleAddConnection(
            args?.name as string,
            args?.host as string,
            (args?.port as number) || 5432,
        ...
        case "remove_connection":
          result = await handleRemoveConnection(args?.name as string);
          break;
        case "list_connections":
          result = handleListConnections();
          break;
        case "test_connection":
          result = await handleTestConnection(args?.connection as string);
          break;
        case "list_schemas":
          result = await handleListSchemas(args?.connection as string);
          break;
        case "list_tables":
        ...
        case "get_table_sample":
          result = await handleGetTableSample(
            args?.connection as string,
            args?.table as string,
            args?.schema as string,
            args?.limit as number
        ...
        case "get_table_count":
          result = await handleGetTableCount(
            args?.connection as string,
            args?.table as string,
            args?.schema as string
          );
        ...
        case "get_foreign_keys":
          result = await handleGetForeignKeys(
            args?.connection as string,
            args?.table as string,
            args?.schema as string
          );
        ...
        case "compare_schemas":
          result = await handleCompareSchemas(
            args?.source_connection as string,
            args?.target_connection as string,
            args?.table as string,
            args?.schema as string
          );
          break;
        case "compare_row_counts":
          result = await handleCompareRowCounts(
            args?.source_connection as string,
            args?.target_connection as string,
